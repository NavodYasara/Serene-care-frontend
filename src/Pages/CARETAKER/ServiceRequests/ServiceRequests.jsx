...
const ServiceRequests = () => {
  ...
  const handleRequestSubmit = async () => {
    if (!requestData.requirement || !requestData.startDate || !requestData.endDate) {
    ...
    <Box sx={{ display: "flex", bgcolor: "#f4f7fe", minHeight: "100vh" }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 4 }}>
        <Container maxWidth="md">
          ...
          >
            <Box sx={{ position: "relative", zIndex: 1 }}>
              ...
              <Typography variant="body1" sx={{ opacity: 0.8 }}>
                Tell us what you need. Submit a new care requirement to find the best caregiver for you.
              </Typography>
            </Box>
            <AssignmentIcon 
                sx={{ 
          ...
          <Card sx={{ borderRadius: 4, boxShadow: "0 4px 20px 0 rgba(0,0,0,0.05)", overflow: 'visible' }}>
            <CardContent sx={{ p: 4 }}>
              <Stack spacing={4}>
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormControl fullWidth>
                                ...
                                <Select
                                    name="caretakerId"
                                    value={requestData.caretakerId}
                                    onChange={handleSelectCaretaker}
                                    label="Select Caretaker"
                                    startAdornment={<PeopleIcon sx={{ mr: 1, color: 'text.secondary' }} />}
                                >
                                    {caretakers.length === 0 ? (
                                        <MenuItem disabled>No caretakers found</MenuItem>
                                    ) : (
                                        caretakers.map((ct) => (
                                            <MenuItem key={ct.caretakerId} value={ct.caretakerId}>
                                                {ct.firstName} {ct.lastName} (ID: {ct.caretakerId})
                                            </MenuItem>
                                        ))
                                    )}
                                </Select>
                ...
                <Box>
                    <Typography variant="h6" fontWeight="bold" sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 1 }}>
                        <EventIcon color="primary" /> Schedule
                    </Typography>
                    <Grid container spacing={3}>
                        <Grid item xs={12} sm={6}>
                            <TextField
                ...
                <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
                    <Button
                        ...
                        sx={{
                            ...
                            '&:hover': {
                                background: "linear-gradient(45deg, #162d55 30%, #1e3c72 90%)",
